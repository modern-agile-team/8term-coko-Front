interface CalculateCycleProgressParams {
  value: number;
  cycleLength?: number;
  step?: number;
}

interface CalculateCycleProgressResult {
  cycle: number;
  steps: number[];
...
/**
 * @description 주어진 값(value)을 기반으로 해당 값이 속한 사이클, 진행도, 및 주요 단계들을 계산하는 함수입니다.
 *
 * @param {CalculateCycleProgressParams} params - 입력 파라미터를 포함하는 객체입니다.
 ...
 *
 * @returns {CalculateCycleProgressResult} 계산된 사이클 정보를 담은 객체를 반환합니다.
 ...
 *
 * @example
 * const { cycle, steps, positionInCycle, progress } = calculateCycleProgress({ value: 125, cycleLength: 100, step: 10 });
 ...
 */
export default function calculateCycleProgress({
  value,
  cycleLength = 100,
  step = 10,
}: CalculateCycleProgressParams): CalculateCycleProgressResult {
